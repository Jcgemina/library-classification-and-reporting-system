...
(() => {
  ...
  const toast = (message, error = false) => { const container = $('courseToastContainer'); const node = document.createElement('div'); const duration = 3000; node.className = `pointer-events-auto relative flex items-start gap-3 overflow-hidden rounded-xl border px-4 py-3 pb-4 text-sm shadow-lg ${error ? 'border-red-300 bg-red-50 text-red-800' : 'border-green-300 bg-green-50 text-green-800'}`; node.innerHTML = `<i data-lucide="${error ? 'circle-alert' : 'circle-check'}" class="mt-0.5 h-4 w-4 flex-shrink-0"></i><span class="flex-1">${esc(message)}</span><button type="button" class="text-current opacity-60 transition hover:opacity-100" aria-label="Dismiss notification"><i data-lucide="x" class="h-4 w-4"></i></button><span class="absolute bottom-0 left-0 h-1 w-full origin-left ${error ? 'bg-red-500' : 'bg-green-500'}" data-toast-progress></span>`; container.appendChild(node); lucide.createIcons(); const progress = node.querySelector('[data-toast-progress]'); requestAnimationFrame(() => { progress.style.transition = `width ${duration}ms linear`; progress.style.width = '0%'; }); const dismiss = () => node.remove(); node.querySelector('button').addEventListener('click', dismiss); setTimeout(dismiss, duration); };
  const request = async (url, options = {}) => { const response = await fetch(`pages/course.php${url}`, { headers: {'X-Requested-With':'XMLHttpRequest', ...(options.body instanceof FormData ? {} : {'Content-Type':'application/x-www-form-urlencoded'})}, ...options }); const data = await response.json(); if (!response.ok || !data.success) throw new Error(data.message || 'Request failed.'); return data; };
    function closeCourseDetails() { $('courseDetailsModal')?.remove(); }
    function openCourseDetails(course) {
        closeCourseDetails();
        const modal = document.createElement('div');
        modal.id = 'courseDetailsModal';
        modal.className = 'fixed inset-0 z-[55]';
        modal.innerHTML = `<button type="button" data-close-course-details class="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" aria-label="Close course details"></button><aside class="absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="courseDetailsTitle"><div class="flex items-start justify-between border-b border-slate-200 px-6 py-5"><div><p class="text-xs font-semibold uppercase tracking-[0.16em] text-rose-600">Course record</p><h3 id="courseDetailsTitle" class="mt-1 text-xl font-bold text-slate-900">${esc(course.code)} - ${esc(course.name)}</h3></div><button type="button" data-close-course-details class="rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-800" aria-label="Close course details"><i data-lucide="x" class="h-4 w-4"></i></button></div><div class="flex-1 overflow-y-auto px-6 py-6"><p class="rounded-xl bg-rose-50 px-4 py-3 text-sm text-rose-800">${esc(course.description || 'No description provided.')}</p><dl class="mt-6 divide-y divide-slate-100 rounded-xl border border-slate-200"><div class="flex justify-between gap-4 px-4 py-3"><dt class="text-sm text-slate-500">Units</dt><dd class="text-right text-sm font-medium text-slate-800">${course.units ?? 'Not set'}</dd></div><div class="flex justify-between gap-4 px-4 py-3"><dt class="text-sm text-slate-500">Type</dt><dd class="text-right text-sm font-medium text-slate-800">${esc(course.type)}</dd></div><div class="flex justify-between gap-4 px-4 py-3"><dt class="text-sm text-slate-500">Year level</dt><dd class="text-right text-sm font-medium text-slate-800">${course.yearLevel ?? 'Not set'}</dd></div><div class="flex justify-between gap-4 px-4 py-3"><dt class="text-sm text-slate-500">College</dt><dd class="text-right text-sm font-medium text-slate-800">${esc(course.collegeName || 'Unassigned')}</dd></div><div class="flex justify-between gap-4 px-4 py-3"><dt class="text-sm text-slate-500">Program</dt><dd class="text-right text-sm font-medium text-slate-800">${esc(course.programName || 'Unassigned')}</dd></div><div class="flex justify-between gap-4 px-4 py-3"><dt class="text-sm text-slate-500">Major</dt><dd class="text-right text-sm font-medium text-slate-800">${esc(course.majorName || 'Unassigned')}</dd></div><div class="flex justify-between gap-4 px-4 py-3"><dt class="text-sm text-slate-500">Status</dt><dd class="text-right text-sm font-medium capitalize text-slate-800">${esc(course.status)}</dd></div></dl></div><div class="border-t border-slate-200 bg-slate-50 px-6 py-4"><div class="flex justify-end gap-2"><button type="button" data-close-course-details class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">Close</button>${admin ? `<button type="button" data-details-delete="${course.id}" class="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-100">Delete</button>` : ''}</div></div></aside>`;
        document.body.appendChild(modal); lucide.createIcons(); modal.querySelectorAll('[data-close-course-details]').forEach(button => button.addEventListener('click', closeCourseDetails)); modal.querySelector('[data-details-delete]')?.addEventListener('click', () => openCourseDelete(course));
    }
    function closeCourseDelete() { $('courseDeleteModal')?.remove(); }
    function openCourseDelete(course) {
        closeCourseDelete();
        const modal = document.createElement('div'); modal.id = 'courseDeleteModal'; modal.className = 'fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm';
        modal.innerHTML = `<div class="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl" role="dialog" aria-modal="true"><div class="flex items-start gap-4"><div class="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-red-100 text-red-600"><i data-lucide="trash-2" class="h-5 w-5"></i></div><div><h3 class="text-lg font-bold text-slate-900">Delete course?</h3><p class="mt-1 text-sm text-slate-600">Delete ${esc(course.code)} - ${esc(course.name)} permanently?</p><p class="mt-2 text-sm font-semibold text-red-600">This action cannot be undone.</p></div></div><label class="mt-5 block text-sm font-semibold text-slate-700">Administrator password<input type="password" data-course-delete-password autocomplete="current-password" class="mt-2 w-full rounded-xl border-2 border-slate-300 px-3 py-2.5 text-sm text-slate-700 focus:border-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-200" placeholder="Enter your password"></label><div class="mt-6 flex justify-end gap-3"><button type="button" data-course-delete-cancel class="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">Cancel</button><button type="button" data-course-delete-confirm class="inline-flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-red-700"><i data-lucide="trash-2" class="h-4 w-4"></i>Delete</button></div></div>`;
        document.body.appendChild(modal); lucide.createIcons(); modal.querySelector('[data-course-delete-cancel]').onclick = closeCourseDelete; modal.querySelector('[data-course-delete-password]').focus(); modal.querySelector('[data-course-delete-confirm]').onclick = async () => { const password = modal.querySelector('[data-course-delete-password]').value; if (!password) { toast('Enter the administrator password to continue.', true); return; } const confirmButton = modal.querySelector('[data-course-delete-confirm]'); confirmButton.disabled = true; confirmButton.innerHTML = '<i data-lucide="loader-circle" class="h-4 w-4 animate-spin"></i>Deleting...'; lucide.createIcons(); try { const data = await request('', {method:'POST', body:new URLSearchParams({action:'delete', id:course.id, current_password:password})}); closeCourseDelete(); closeCourseDetails(); toast(data.message); await loadCourses(); } catch (error) { confirmButton.disabled = false; confirmButton.innerHTML = '<i data-lucide="trash-2" class="h-4 w-4"></i>Delete'; lucide.createIcons(); toast(error.message, true); } };
    }
    const courseBodyObserver = new MutationObserver(() => { $('courseBody')?.querySelectorAll('tr').forEach((row, index) => { if (row.querySelector('[data-view]')) return; const course = state.courses[index]; if (!course) return; const actions = row.lastElementChild?.querySelector('div'); if (!actions) return; const button = document.createElement('button'); button.type = 'button'; button.dataset.view = String(course.id); button.className = 'inline-flex items-center gap-1.5 rounded-md border border-sky-200 bg-sky-50 px-2.5 py-1.5 text-xs font-semibold text-sky-700 hover:bg-sky-100'; button.innerHTML = '<i data-lucide="eye" class="h-3.5 w-3.5"></i>View'; actions.prepend(button); lucide.createIcons(); }); });
    function syncCourseProgramOptions() { const collegeId = $('courseCollege').value; const selectedProgram = $('courseProgram').value; const programs = state.programs.filter((program) => !collegeId || String(program.college_id) === String(collegeId)); const validProgram = programs.some((program) => String(program.id) === String(selectedProgram)) ? selectedProgram : ''; fillSelect($('courseProgram'), programs, 'All programs', validProgram); }
  ...
  function render() { $('courseBody').innerHTML = state.courses.length ? state.courses.map((course) => `<tr class="align-top hover:bg-slate-50"><td class="px-3 py-4"><div class="font-bold text-slate-900">${esc(course.code)}</div><div class="mt-1 text-slate-600">${esc(course.name)}</div></td><td class="px-3 py-4 text-slate-600"><span>${course.units !== null ? `${course.units} unit${course.units === 1 ? '' : 's'}` : 'Units not set'}</span><span class="mx-1 text-slate-300">|</span><span>${esc(course.type)}</span>${course.yearLevel ? `<div class="mt-1 text-xs text-slate-500">Year ${course.yearLevel}</div>` : ''}</td><td class="px-3 py-4">${course.programName ? `<div class="font-medium text-slate-800">${esc(course.programName)}</div><div class="text-xs text-slate-500">${esc(course.collegeName)}${course.majorName ? ` / ${esc(course.majorName)}` : ''}</div>` : '<span class="text-xs italic text-slate-400">Unassigned</span>'}</td><td class="px-3 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-bold ${course.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}">${esc(course.status)}</span></td><td class="px-3 py-4 text-right"><div class="flex flex-wrap justify-end gap-2">${admin ? `<button type="button" data-edit="${course.id}" class="rounded-md border border-slate-300 px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:border-rose-400 hover:text-rose-700">Edit</button><button type="button" data-toggle="${course.id}" class="rounded-md border border-slate-300 px-2.5 py-1.5 text-xs font-semibold text-slate-700">${course.status === 'active' ? 'Deactivate' : 'Activate'}</button><button type="button" data-unlink="${course.id}" class="rounded-md border border-amber-300 px-2.5 py-1.5 text-xs font-semibold text-amber-700">Unlink</button><button type="button" data-delete="${course.id}" class="rounded-md border border-red-200 px-2.5 py-1.5 text-xs font-semibold text-red-600">Delete</button>` : '<span class="text-xs text-slate-400">View only</span>'}</div></td></tr>`).join('') : '<tr><td colspan="5" class="px-3 py-12 text-center text-sm text-slate-500">No courses match the selected filters.</td></tr>'; }
    ...
    async function loadCourses() { syncCourseProgramOptions(); const requestId = ++latestCourseRequest; try { const data = await request(params()); if (requestId !== latestCourseRequest) return; state.courses = data.courses; $('totalCount').textContent = data.counts.total; $('activeCount').textContent = data.counts.active; $('inactiveCount').textContent = data.counts.inactive; render(); $('courseStatusMessage').textContent = `${state.courses.length} shown`; } catch (error) { if (requestId !== latestCourseRequest) return; $('courseStatusMessage').textContent = error.message; toast(error.message, true); } }
